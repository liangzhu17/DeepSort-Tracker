...
def create_network(images, num_classes=None, add_logits=True, reuse=None,
    ...
    if add_logits:
        print("slim.model_variable:",slim.model_variable)

        with tf.compat.v1.variable_scope('ball',reuse=reuse):  #slim.variable_scope.variable_scope('ball', reuse=reuse)
            weights = slim.model_variable(
                    "mean_vectors", (feature_dim, int(num_classes)), ###(128,770)
...
def create_network_factory(is_training, num_classes, add_logits,
    ...
    def factory_fn(image):
            with slim.arg_scope([slim.batch_norm, slim.dropout],           #num_class=770
