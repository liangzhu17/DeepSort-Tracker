...
class _BaseDataset(ABC):
    ...
    """@staticmethod
    def _calculate_box_polygon_ious(polygon1, polygon2, box_format='xy3214', do_ioa=False):
        if box_format in 'xy3214':
            # layout: (x3, y3, x2, y2, x1, y1, x4, y4)
            pol1 = Polygon(polygon1)
            pol2 = Polygon(polygon2)
            polygon_1 = deepcopy(pol1)
            polygon_2 = deepcopy(pol2)
        ...
        elif box_format not in 'x3y3x2y2x1y1x4y4':
        ...
        if do_ioa:
            ...
            return ious"""
    @staticmethod
    def _calculate_box_ious(gt_w, t_w):

        cost_matrix = np.zeros((len(gt_w), len(t_w)))
        pol1 = gt_w[:, [[4, 5], [2, 3], [0, 1], [6, 7]]]
        pol2 = t_w[:, [[0, 1], [2, 3], [4, 5], [6, 7]]]

        for row in range(len(pol1)):  # row index
            pol_gt_xy4 = Polygon(pol1[row])
            # pol_track_xy4 = np.asarray([Polygon(pol2[i]) for i in range(len(pol1))])

            iou_list = []
            for idx in range(len(pol2)):
                p_track = Polygon(pol2[idx])
                # Calculate intersection and union, and the IOU
                polygon_intersection = pol_gt_xy4.intersection(p_track).area
                polygon_union = pol_gt_xy4.area + p_track.area - polygon_intersection
    ...
    @staticmethod
    def _iou_cost_bbox_camera(bboxes1, bboxes2, box_format='xywh', do_ioa=False):
