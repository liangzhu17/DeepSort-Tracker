...
class MotChallenge2DBox(_BaseDataset):
    ...
    @staticmethod
    def get_default_dataset_config(gt_fol, tracker_fol, out_fol, seq_info, seqmap_file):
        """Default class config values"""
        code_path = utils.get_code_path()
        default_config = {
            'GT_FOLDER': gt_fol,  # Location of GT data
            'TRACKERS_FOLDER': tracker_fol,
            # os.path.join(code_path, 'eval_data/trackers/'),  # Trackers location
            'OUTPUT_FOLDER': out_fol,  # Where to save eval results (if None, same as TRACKERS_FOLDER)
            ...
            'GT_LOC_FORMAT': '{gt_folder}/{seq}/gt/gt_eval.txt',  # '{gt_folder}/{seq}/gt/gt.txt'
    ...
    def __init__(self, gt_fol, tracker_fol, out_fol, seq_info, seqmap_file, config=None):
        """Initialise dataset, checking that all required files are present"""
        super().__init__()
        # Fill non-given config values with defaults
        self.config = utils.init_config(config,
                                        self.get_default_dataset_config(gt_fol, tracker_fol, out_fol, seq_info,
                                                                        seqmap_file), self.get_name())
        ...
        self.output_root = self.config['OUTPUT_FOLDER']
        # Get classes to eval
        self.valid_classes = ['car']
        self.output_fol = osp.join(self.output_root + "/", self.valid_classes[0])
        self.class_list = [cls.lower() if cls.lower() in self.valid_classes else None
                           for cls in self.config['CLASSES_TO_EVAL']]
        if not all(self.class_list):
            raise TrackEvalException('Attempted to evaluate an invalid class. Only car class is valid.')
        self.class_name_to_class_id = {'car': 1, 'truck': 2, 'van': 3, 'bus': 4}
        self.valid_class_numbers = list(self.class_name_to_class_id.values())

    # Get sequences to eval and check gt files exist
        self.seq_list, self.seq_lengths = self._get_seq_info(seq_info, seqmap_file)
    ...
    def _get_seq_info(self, seq_info, seqmap_file):
        seq_list = []
        seq_lengths = {}
        if self.config["SEQ_INFO"]:
            ...
            if self.config["SEQMAP_FILE"]:
                #seqmap_file = list(self.config.keys())[list(self.config.values()).index()]
                seqmap_file = seqmap_file
            else:
            ...
            with open(seqmap_file) as fp:
                ...
                for i, row in enumerate(reader):
                    ...
                    if i == 1:
                        ...
                        seq_list.append(seq)
                        print(seq_list)
                        ini_file = seq_info  # seq_info .ini file
                        if not os.path.isfile(ini_file):
                            raise TrackEvalException(
    ...
    def _load_raw_file(self, tracker, seq, is_gt):
        ...
        [tracker_dets]: list (for each timestep) of lists of detectiogins.
        """
        # File location
        if self.data_is_zipped:
            if is_gt:
        ...
        for t in range(num_timesteps):
                    ...
                    if not is_gt:
                        ...
                        raise TrackEvalException(
                            'GT data is not in a valid format, there is not enough rows in seq %s, timestep %i.' % (
                                seq, t))
                if is_gt:
                    gt_extras_dict = {'zero_marked': np.atleast_1d(time_data[:, 10].astype(int))}
            ...
            if is_gt:
                raw_data['gt_crowd_ignore_regions'][t] = np.empty((0, 8))
    ...
    def get_preprocessed_seq_data(self, raw_data, cls):
        ...
        distractor_class_names = ['truck', 'van', 'bus']
    ...
    """def _calculate_polygon_similarities(self, gt_dets_t, tracker_dets_t):
        similarity_scores = self._calculate_box_polygon_ious(gt_dets_t, tracker_dets_t, box_format='xywh')
        return similarity_scores"""