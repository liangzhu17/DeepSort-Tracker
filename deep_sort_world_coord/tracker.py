...
class Tracker:
    ...
    def _match(self, detections):
        ...
        unconfirmed_tracks = [
            i for i, t in enumerate(self.tracks) if not t.is_confirmed()]  # when empty, no new car comes in view form last fid
        # here the unconfirmed_tracks should also take part in matching cascade
        if len(total_tracks) == 0:
            return [],[], detections

        else:  # total_tracks not empty
            ...
            matches_b, unmatched_tracks_b, unmatched_detections = \
                linear_assignment.iou_min_cost_matching(
                    iou_matching.iou_cost_polygon, self.max_iou_distance, self.tracks,
    ...
    def _initiate_track(self, detection):
        mean, covariance = self.kf.initiate(detection.to_w_xyaw())
        self.tracks.append(Track(
            mean, covariance, self._next_id, self.n_init, self.max_age, detection.rot_alpha,
